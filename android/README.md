# DEAR Management Channel — Aplikasi Android (untuk Play Store / instal manual)
...
Android DEAR Management (HR) — package name, keystore penandatanganan, dan
domain yang dibungkus semuanya berbeda, sehingga tidak akan pernah saling
bentrok di HP yang sama (keduanya bisa terinstal bersamaan).

## 1. Tambahkan 2 secret di GitHub (sekali saja)

Buka repo `ar-client-dear-management` di GitHub → **Settings → Secrets and
variables → Actions → New repository secret**, lalu tambahkan:

| Nama secret | Isi |
|---|---|
| `ANDROID_KEYSTORE_BASE64` | Isi file `dear-channel-keystore-base64.txt` yang saya kirimkan terpisah (bukan lewat git, karena ini rahasia) |
| `ANDROID_KEYSTORE_PASSWORD` | Password dari file `dear-channel-keystore-credentials.txt` yang saya kirimkan terpisah |

**PENTING:** simpan file `android.keystore` dan
`dear-channel-keystore-credentials.txt` yang saya kirimkan ke Anda di
...
install count hilang). Keystore ini khusus untuk DEAR Management Channel —
jangan dicampur dengan keystore aplikasi DEAR Management (HR) yang lain.
...
scroll ke bagian **Artifacts**, unduh `dear-channel-android-<nomor>.zip`.
Isinya:
...
APK terbaru juga otomatis dipublikasikan ke `public/downloads/dear-channel.apk`
