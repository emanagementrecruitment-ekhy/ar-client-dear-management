...
async function sendOtpEmailViaBrevoApi(to: string, code: string) {
  ...
  const res = await fetch("https://api.brevo.com/v3/smtp/email", {
    ...
    body: JSON.stringify({
      sender: { email: sender.email, name: sender.name || "DEAR Management Channel" },
      to: [{ email: to }],
      subject: "Kode verifikasi DEAR Management Channel",
...
export async function sendOtpWhatsapp(toPhoneDigits: string, code: string) {
  ...
  const body = new URLSearchParams({
    target,
    message: `Kode verifikasi DEAR Management Channel Anda: ${code} (berlaku 5 menit). Jangan bagikan kode ini kepada siapa pun.`,
  });
