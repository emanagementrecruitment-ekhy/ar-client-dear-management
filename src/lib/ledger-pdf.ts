...
const LOGO_PATH = path.join(process.cwd(), "public", "dear-management-logo.png");
 ...
 * faint centered watermark of the DEAR Management logo is drawn first (low
 * opacity, behind every other element) so a printed/forwarded copy is still
 * recognizably an official DEAR Management document. */
export function generateLedgerPdf(input: LedgerPdfInput): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    ...
    doc.fontSize(9).fillColor("#8a6d1f").font("Helvetica-Bold").text("DEAR MANAGEMENT CHANNEL", { characterSpacing: 2 });
    doc.moveDown(0.2);
